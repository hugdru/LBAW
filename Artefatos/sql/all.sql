...
BEGIN
  ...
  (SELECT idEvento, titulo, descricao, capa, localizacao, dataInicio
    ...
    WHERE
idEvento in (
  select participacao.idevento from participacao where participacao.idparticipante = idutilizadorrr union select convite.idevento from convite where convite.idconvidado = idutilizadorrr union select anfitriao.idevento from anfitriao where anfitriao.idanfitriao = idutilizadorrr union select evento.idevento from evento where evento.publico = true) and
      Event_view_fts.document @@ plainto_tsquery(texto)
    ORDER BY ts_rank(Event_view_fts.document, plainto_tsquery(texto)) DESC) AS search;
    return result;
END;
...
WHERE idEvento in (
  select participacao.idevento from participacao where participacao.idparticipante = idutilizadorrr union select convite.idevento from convite where convite.idconvidado = idutilizadorrr union select anfitriao.idevento from anfitriao where anfitriao.idanfitriao = idutilizadorrr union select evento.idevento from evento where evento.publico = true)
) AS search;
return result;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  return exists (
    SELECT * FROM
    (select participacao.idevento from participacao where participacao.idparticipante = idutilizadorrr union select convite.idevento from convite where convite.idconvidado = idutilizadorrr union select anfitriao.idevento from anfitriao where anfitriao.idanfitriao = idutilizadorrr union select evento.idevento from evento where evento.publico = true) AS search
