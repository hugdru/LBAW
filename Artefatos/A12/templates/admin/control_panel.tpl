        ...
        <table class="table">
            ...
            {foreach from=$list item='item'}
                <tr>
                    ...
                    <td>{$item['email']}</td>
                    <td>
                        <form onsubmit="return confirm('Do you really want to delete this user and all its contents?');"
                              action="{$action_user_remove}" method="post">
                            <input type="hidden" name="idutilizador" value="{$item['idutilizador']}">
                            <button class="btn btn-default btn-xs" type="submit"><span
        ...
        <table class="table">
            ...
            {foreach from=$list item='item'}
                <tr>
                    ...
                    <td>{if $item['publico'] eq 1}
                            <span class="glyphicon glyphicon-eye-open"></span>
                            Public{else}
                            <span class="glyphicon glyphicon-eye-close"></span>
                            Private{/if}</td>
                    <td>
                        <form onsubmit="return confirm('Do you really want to delete this event and all its contents?');"
                              action="{$action_event_remove}" method="post">
                            <input type="hidden" name="idevento" value="{$item['idevento']}">
                            <a class="btn btn-default btn-xs"
                               href="{$BASE_URL}pages/event/view_event.php?id={$item['idevento']}"><span
        ...
        <table class="table">
            ...
            {foreach from=$list item='item'}
                <tr>
                    ...
                    <td>
                        <form onsubmit="return confirm('Do you really want to delete this comment and its nested comments?');"
                              action="{$action_comment_remove}" method="post">
                            <input type="hidden" name="idcomentario" value="{$item['idcomentario']}">
                            <a class="btn btn-default btn-xs"
                               href="{$BASE_URL}pages/event/view_event.php?id={$item['idevento']}"><span
                                        class="glyphicon glyphicon-search"></span> View Context</a>
                            <button class="btn btn-default btn-xs" type="submit"><span
                                        class="glyphicon glyphicon-remove"></span> Remove
                            </button>
                        </form>
...
{if $option eq 4}
    <div class="panel_list">
        <div class="row">
            <div class="col-sm-4">
                <form method="post" action="{$action_create_admin}" role="form"
                      onsubmit="return confirm('Creating a new Administrator account, Continue?');">
